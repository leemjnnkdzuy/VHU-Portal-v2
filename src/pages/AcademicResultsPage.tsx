...
import { Loader2, AlertCircle, BookOpen, GraduationCap, TrendingUp, Award, ChevronDown, ChevronUp, Check, X, List, Table, BarChart3 } from 'lucide-react';
...
function AcademicResultsPage() {
        ...
        const fetchResults = async () => {
            ...
            try {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    const latestSemester = latestYear?.DanhSachDiem?.[latestYear.DanhSachDiem.length - 1];
                    const latestCourse = latestSemester?.DanhSachDiemHK?.[0];

                    if (latestCourse) {
                        setGPA({
                            semesterGPA10: parseFloat(latestCourse.TB_HK || '0'),
                            semesterGPA4: parseFloat(latestCourse.TB_HK4 || '0'),
                            cumulativeGPA10: parseFloat(latestCourse.TB_TL_HK || '0'),
                            cumulativeGPA4: parseFloat(latestCourse.TB_TL_HK4 || '0'),
                            creditsEarned: parseInt(latestCourse.Dat_TL_HK || '0'),
                            totalCreditsRegistered: parseInt(latestCourse.TongTC_DK_HK || '0'),
                        });
                    }
                }
            } catch (err) {
    ...
    const getSemesterLabel = (hocKy: string) => {
        switch (hocKy) {
            case 'HK01': return 'Học kỳ 1';
            case 'HK02': return 'Học kỳ 2';
            case 'HK03': return 'Học kỳ Hè';
            default: return hocKy;
        }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div className="flex items-center gap-1 p-1 bg-muted rounded-lg">
                    <Button
                        ...
                        className={cn(
                            ...
                            viewMode === 'program' ? "shadow-sm" : "hover:bg-background/50"
                        )}
                    >
                        <List className="w-4 h-4" />
                        <span className="hidden sm:inline">Theo lộ trình</span>
                    </Button>
                    <Button
                        variant={viewMode === 'curriculum' ? 'default' : 'ghost'}
                        size="sm"
                        onClick={() => setViewMode('curriculum')}
                        disabled={isLoading}
                        className={cn(
                            "gap-2 transition-all",
                            viewMode === 'curriculum' ? "shadow-sm" : "hover:bg-background/50"
                        )}
                    >
                        <Table className="w-4 h-4" />
                        <span className="hidden sm:inline">Theo chương trình</span>
                    </Button>
                </div>
                ...
                >
                    ...
                    <span className="hidden sm:inline">{activeTab === 'statistics' ? 'Xem kết quả' : 'Thống kê'}</span>
                </Button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card className="border-0 shadow-lg bg-gradient-to-br from-primary to-primary/80 text-primary-foreground">
                    <CardContent className="p-5">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-primary-foreground/70 text-xs">GPA Tích lũy</p>
                                <div className="flex items-baseline gap-2 mt-1">
                                    <p className="text-3xl font-bold">
                                        {gpa.cumulativeGPA4?.toFixed(2) || '—'}
                                    </p>
                                    <span className="text-sm text-primary-foreground/80">/4.0</span>
                                </div>
                                <p className="text-primary-foreground/70 text-xs mt-1">
                                    ({gpa.cumulativeGPA10?.toFixed(2) || '—'}/10)
                                </p>
                            </div>
                ...
                <Card className="border shadow-lg">
                    <CardContent className="p-5">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-muted-foreground text-xs">GPA Học kỳ</p>
                                <div className="flex items-baseline gap-2 mt-1">
                ...
                <Card className="border shadow-lg">
                    <CardContent className="p-5">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-muted-foreground text-xs">Tín chỉ tích lũy</p>
                                <p className="text-3xl font-bold mt-1 text-foreground">
                                    {gpa.creditsEarned || '—'}
                                </p>
                                <p className="text-muted-foreground text-xs mt-1">Đã đạt</p>
                            </div>
                            <div className="p-3 bg-muted rounded-full">
                ...
                <Card className="border shadow-lg">
                    <CardContent className="p-5">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-muted-foreground text-xs">Tổng số năm học</p>
                                <p className="text-3xl font-bold mt-1 text-foreground">
                                    {yearlyResults.length || '—'}
                                </p>
                                <p className="text-muted-foreground text-xs mt-1">Năm học</p>
                            </div>
                            <div className="p-3 bg-muted rounded-full">
                            ...
                            <CardContent className="p-0">
                                <div className="divide-y divide-border">
                                    {year.DanhSachDiem?.map((semester) => {
                                        ...
                                        const passedCourses = semester.DanhSachDiemHK?.filter(c => c.IsPass === '1').length || 0;
                                            ...
                                            <div key={key}>
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="text-left">
                                                            <p className="font-semibold text-foreground">
                                                                {getSemesterLabel(semester.HocKy)}
                                                            </p>
                                                ...
                                                {isExpanded && (
                                                    <div className="px-4 pb-4">
                                                            ...
                                                            <table className="w-full">
                                                                <thead>
                                                                    <tr className="bg-muted/50">
                                                                        <th className="text-left py-2.5 px-3 text-xs font-semibold text-muted-foreground">Mã môn</th>
                                                                        <th className="text-left py-2.5 px-3 text-xs font-semibold text-muted-foreground">Tên môn học</th>
                                                                        <th className="text-center py-2.5 px-3 text-xs font-semibold text-muted-foreground">TC</th>
                                                                        <th className="text-center py-2.5 px-3 text-xs font-semibold text-muted-foreground">Điểm 10</th>
                                                                        <th className="text-center py-2.5 px-3 text-xs font-semibold text-muted-foreground">Điểm 4</th>
                                                                        <th className="text-center py-2.5 px-3 text-xs font-semibold text-muted-foreground">Điểm chữ</th>
                                                                        <th className="text-center py-2.5 px-3 text-xs font-semibold text-muted-foreground">Đạt</th>
                                                                    </tr>
                                                                </thead>
                                                            ...
                                                            {semester.DanhSachDiemHK?.map((course, idx) => (
                                                                <div
                                                                    ...
                                                                    className={cn(
                                                                        ...
                                                                        course.IsPass === '0' && "border-red-500/50 bg-red-500/5"
                                                                    )}
                                                                >
                                                                    ...
                                                                    <div className="flex items-center gap-3 text-xs text-muted-foreground pt-2 border-t border-border/50">
                                                                        ...
                                                                        <span>Điểm 10: <span className="font-medium text-foreground">{course.DiemTK_10}</span></span>
                                                                        <span>Điểm 4: <span className="font-medium text-foreground">{course.DiemTK_4}</span></span>
                                                                        {course.IsPass === '1' ? (
                                                                            <span className="text-green-600 dark:text-green-400 ml-auto">Đạt</span>
                                                                        ) : (
                                                                            <span className="text-red-600 dark:text-red-400 ml-auto">Chưa đạt</span>
                                                                        )}
                                                                    </div>
                                                                </div>
                                                            ))}
                                                        ...
                                                        {/* Semester Summary */}
                                                        {semester.DanhSachDiemHK?.[0] && (
                                                            <div className="mt-3 p-3 rounded-lg bg-muted/50 flex flex-wrap gap-4 text-sm">
                                                                <div>
                                                                    <span className="text-muted-foreground">GPA HK: </span>
                                                                    <span className="font-semibold">{semester.DanhSachDiemHK[0].TB_HK4}/4.0</span>
                                                                    <span className="text-muted-foreground text-xs ml-1">({semester.DanhSachDiemHK[0].TB_HK}/10)</span>
                                                                </div>
                                                                <div>
                                                                    <span className="text-muted-foreground">GPA TL: </span>
                                                                    <span className="font-semibold">{semester.DanhSachDiemHK[0].TB_TL_HK4}/4.0</span>
                                                                    <span className="text-muted-foreground text-xs ml-1">({semester.DanhSachDiemHK[0].TB_TL_HK}/10)</span>
                                                                </div>
                                                                <div>
                                                                    <span className="text-muted-foreground">TC tích lũy: </span>
                                                                    <span className="font-semibold">{semester.DanhSachDiemHK[0].Dat_TL_HK}</span>
                                                                </div>
                                                            </div>
                                                        )}
                                                    </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="p-3 rounded-lg border bg-card">
                                    ...
                                    <p className="font-medium text-foreground">{selectedCourse.CurriculumID}</p>
                                </div>
                                <div className="p-3 rounded-lg border bg-card">
                                    <p className="text-xs text-muted-foreground">Số tín chỉ</p>
                                    <p className="font-medium text-foreground">{selectedCourse.Credits} TC</p>
                                </div>
                                {selectedCourse.ScheduleStudyUnitID && (
                                    <div className="p-3 rounded-lg border bg-card col-span-2">
                                        <p className="text-xs text-muted-foreground">Mã lớp học phần</p>
                                        <p className="font-medium text-foreground">{selectedCourse.ScheduleStudyUnitID}</p>
                                    </div>
                                )}
                            </div>
                            ...
                            {/* Additional Info */}
                            {(selectedCourse.ListOfProfessorName || selectedCourse.Note) && (
                                <div className="space-y-2 pt-2 border-t">
                                    {selectedCourse.ListOfProfessorName && (
                                        <div>
                                            <p className="text-xs text-muted-foreground">Giảng viên</p>
                                            <p className="text-sm text-foreground">{selectedCourse.ListOfProfessorName}</p>
                                        </div>
                                    )}
                                    {selectedCourse.Note && (
                                        <div>
